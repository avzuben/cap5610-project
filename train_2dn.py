...
np.save('./output-2dn/' + VAL_SET + '/last-weights', model.get_weights())
np.save('./output-2dn/' + VAL_SET + '/history', history.history)
...
def save_image(img, pat, name):
    ...
    im.convert('RGB').save('./output-2dn/' + VAL_SET + '/' + pat + '/' + name + '.jpg', 'JPEG')


for pat in val_patients:
    Path('./output-2dn/' + VAL_SET + '/' + str(pat).zfill(3)).mkdir(parents=True, exist_ok=True)
    ...
    np.save('./output-2dn/' + VAL_SET + '/' + str(pat).zfill(3) + '.npy', pred)
