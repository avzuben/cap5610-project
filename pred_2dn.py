...
for pat in val_patients:
    ...
    weight_path = './output-2dn/' + VAL_SET + '/best-weights'
    ...
    def save_image(img, pat, name):
        ...
        im.convert('RGB').save('./pred-2dn/' + VAL_SET + '/' + pat + '/' + name + '.jpg', 'JPEG')


    Path('./pred-2dn/' + VAL_SET + '/' + str(pat).zfill(3)).mkdir(parents=True, exist_ok=True)
    ...
    for i in range(len(X)):
        ...
        pred_mi_original = np.zeros(
            (y_heart_locator_predicted.shape[0], HEIGHT, WIDTH, y_heart_locator_predicted.shape[3]))
        pred_mi_original[:, yc_heart_locator - HEIGHT_UNET // 2:yc_heart_locator + HEIGHT_UNET // 2,
        xc_heart_locator - WIDTH_UNET // 2:xc_heart_locator + WIDTH_UNET // 2, :] = y_heart_locator_predicted

        pred_mi_horizontal = np.zeros(
            (y_heart_locator_predicted.shape[0], HEIGHT, WIDTH, y_heart_locator_predicted.shape[3]))
        pred_mi_horizontal[:, yc_heart_locator - HEIGHT_UNET // 2:yc_heart_locator + HEIGHT_UNET // 2,
        xc_heart_locator - WIDTH_UNET // 2:xc_heart_locator + WIDTH_UNET // 2, :] = yh_heart_locator_predicted

        pred_mi_vertical = np.zeros(
            (y_heart_locator_predicted.shape[0], HEIGHT, WIDTH, y_heart_locator_predicted.shape[3]))
        pred_mi_vertical[:, yc_heart_locator - HEIGHT_UNET // 2:yc_heart_locator + HEIGHT_UNET // 2,
        xc_heart_locator - WIDTH_UNET // 2:xc_heart_locator + WIDTH_UNET // 2, :] = yv_heart_locator_predicted

        pred_mi_horizontal_vertical = np.zeros(
            (y_heart_locator_predicted.shape[0], HEIGHT, WIDTH, y_heart_locator_predicted.shape[3]))
        pred_mi_horizontal_vertical[:, yc_heart_locator - HEIGHT_UNET // 2:yc_heart_locator + HEIGHT_UNET // 2,
        xc_heart_locator - WIDTH_UNET // 2:xc_heart_locator + WIDTH_UNET // 2, :] = yhv_heart_locator_predicted
    ...
    np.save('./pred-2dn/' + VAL_SET + '/' + str(pat).zfill(3) + '-multi-input-heart-locator.npy', pred_multi_input_heart_locator)
    np.save('./pred-2dn/' + VAL_SET + '/' + str(pat).zfill(3) + '-multi-input-original.npy', pred_multi_input_original)
    np.save('./pred-2dn/' + VAL_SET + '/' + str(pat).zfill(3) + '-multi-input-horizontal.npy', pred_multi_input_horizontal)
    np.save('./pred-2dn/' + VAL_SET + '/' + str(pat).zfill(3) + '-multi-input-vertical.npy', pred_multi_input_vertical)
    np.save('./pred-2dn/' + VAL_SET + '/' + str(pat).zfill(3) + '-multi-input-horizontal-vertical.npy', pred_multi_input_horizontal_vertical)
