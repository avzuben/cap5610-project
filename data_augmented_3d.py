...
class DataGenerator(Sequence):
    ...
    def _generate_X(self, list_temp, x_offset, y_offset):
        ...
        for i, sample in enumerate(list_temp):
            ...
            for j, img in enumerate(sample['data']):
                if img is not None:
                    data[j, ] = self._crop_image((np.array(Image.open(img, 'r')) / 127.5 - 1)[:, :, :1], sample['xc'] + x_offset[i], sample['yc'] + y_offset[i])
            X[i,] = data
        return X
    ...
    def _generate_y(self, list_temp, x_offset, y_offset):
        ...
        for i, sample in enumerate(list_temp):
            # Store mask
            data = np.empty((*self.dim, self.n_classes))
            for j, img in enumerate(sample['mask']):
                if img is not None:
                    data[j, ] = self._crop_image(self._convert_seg_image_to_one_hot_encoding(mask_image_to_class(np.array(Image.open(img, 'r')) / 255.)), sample['xc'] + x_offset[i], sample['yc'] + y_offset[i])
                else:
                    data[j, ] = self._convert_seg_image_to_one_hot_encoding(np.zeros((self.height, self.width)))
            y[i,] = data
        return y
    ...
    def _get_data(self, type):
        ...
        for key in keys:
            for inst in ds[str(key)]:
                ...
                if len(inst['ed']) > self.slices:
                    data = []
                    mask = []
                    for i in range(self.slices):
                        data.append(self.data_dir + inst['ed'][i]['data'])
                        mask.append(self.data_dir + inst['ed'][i]['gt'])
                    dataset.append({
                        'data': data,
                        'mask': mask,
                        'xc': xc,
                    ...
                    data = []
                    mask = []
                    for i in range(len(inst['ed']) - self.slices, len(inst['ed'])):
                        data.append(self.data_dir + inst['ed'][i]['data'])
                        mask.append(self.data_dir + inst['ed'][i]['gt'])
                    dataset.append({
                        'data': data,
                    ...
                    mask = []
                    for i in range(self.slices):
                        if i < (len(inst['ed']) - self.slices) / 2 or i >= len(inst['ed']) + ((len(inst['ed']) - self.slices) / 2):
                            data.append(None)
                            mask.append(None)
                        else:
                            data.append(self.data_dir + inst['ed'][i]['data'])
                ...
                if len(inst['es']) > self.slices:
                    data = []
                    mask = []
                    for i in range(self.slices):
                        data.append(self.data_dir + inst['es'][i]['data'])
                        mask.append(self.data_dir + inst['es'][i]['gt'])
                    dataset.append({
                        'data': data,
                        'mask': mask,
                        'xc': xc,
                    ...
                    data = []
                    mask = []
                    for i in range(len(inst['es']) - self.slices, len(inst['es'])):
                        data.append(self.data_dir + inst['es'][i]['data'])
                        mask.append(self.data_dir + inst['es'][i]['gt'])
                    dataset.append({
                        'data': data,
                    ...
                    mask = []
                    for i in range(self.slices):
                        if i < (len(inst['es']) - self.slices) / 2 or i >= len(inst['es']) + ((len(inst['es']) - self.slices) / 2):
                            data.append(None)
                            mask.append(None)
                        else:
                            data.append(self.data_dir + inst['es'][i]['data'])
