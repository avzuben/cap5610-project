#
# This code was translated to TF and adapted from https://github.com/MIC-DKFZ/ACDC2017
#
...
def build_unet(n_input_channels=1, n_output_classes=4, input_dim=(352, 352), base_n_filters=48, dropout=0.3, pad='same', kernel_size=3, seg=False):
    net = OrderedDict()
    name = 'input'
    ...
    prev = name
    name = 'contr_1_1'
    net[name] = Conv2D(base_n_filters, kernel_size, padding=pad, name="{}_conv".format(name))(net[prev])
    ...
    prev = name
    name = 'contr_2_1'
    net[name] = Conv2D(base_n_filters*2, kernel_size, padding=pad, name="{}_conv".format(name))(net[prev])
    net[name] = BatchNormalization(name="{}_bat_norm".format(name))(net[name])
    ...
    name = 'contr_3_1'
    net[name] = Conv2D(base_n_filters*4, kernel_size, padding=pad, name="{}_conv".format(name))(x)
    net[name] = BatchNormalization(name="{}_bat_norm".format(name))(net[name])

    prev = name
    name = 'contr_3_2'
    net[name] = Conv2D(base_n_filters*4, kernel_size, padding=pad, name="{}_conv".format(name))(net[prev])
    net[name] = BatchNormalization(name="{}_bat_norm".format(name))(net[name])
    ...
    name = 'contr_4_1'
    net[name] = Conv2D(base_n_filters*8, kernel_size, padding=pad, name="{}_conv".format(name))(x)
    net[name] = BatchNormalization(name="{}_bat_norm".format(name))(net[name])

    prev = name
    name = 'contr_4_2'
    net[name] = Conv2D(base_n_filters*8, kernel_size, padding=pad, name="{}_conv".format(name))(net[prev])
    net[name] = BatchNormalization(name="{}_bat_norm".format(name))(net[name])
    ...
    name = 'encode_1'
    net[name] = Conv2D(base_n_filters*16, kernel_size, padding=pad, name="{}_conv".format(name))(x)
    net[name] = BatchNormalization(name="{}_bat_norm".format(name))(net[name])

    prev = name
    name = 'encode_2'
    net[name] = Conv2D(base_n_filters*16, kernel_size, padding=pad, name="{}_conv".format(name))(net[prev])
    net[name] = BatchNormalization(name="{}_bat_norm".format(name))(net[name])
    ...
    name = 'expand_3_1'
    net[name] = Conv2D(base_n_filters*2, kernel_size, padding=pad, name="{}_conv".format(name))(x)
    net[name] = BatchNormalization(name="{}_bat_norm".format(name))(net[name])
    ...
    ds2_1x1_conv = Conv2D(n_output_classes, 1, padding='same', name='ds2_1x1_conv')(ds2)
    ds1_ds2_sum_upscale = UpSampling2D(size=(2, 2), name='ds1_ds2_sum_upscale')(ds2_1x1_conv)
    ds3_1x1_conv = Conv2D(n_output_classes, 1, padding='same', name='ds3_1x1_conv')(net['expand_3_2'])
