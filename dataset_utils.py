...
#
# This code was adapted from https://github.com/MIC-DKFZ/ACDC2017
#
...
def process_patient(args):
    ...
    if os.path.isfile(fname):
        # images["ed"] = np.array(Image.open(fname))
        # images["ed"] = sitk.GetArrayFromImage(sitk.ReadImage(fname))
    ...
    if os.path.isfile(fname):
        # images["ed_seg"] = np.array(Image.open(fname))
        # images["ed_seg"] = sitk.GetArrayFromImage(sitk.ReadImage(fname))
    ...
    if os.path.isfile(fname):
        # images["es"] = np.array(Image.open(fname))
        # images["es"] = sitk.GetArrayFromImage(sitk.ReadImage(fname))
    ...
    if os.path.isfile(fname):
        # images["es_seg"] = np.array(Image.open(fname))
        # images["es_seg"] = sitk.GetArrayFromImage(sitk.ReadImage(fname))
...
def generate_test_patient_info(folder, patients=[]):
    if len(patients) == 0:
        patients = range(101, 151)
    patient_info={}
    ...
    for id in patients:
        ...
        ed_image = sitk.ReadImage(
            os.path.join(folder, "patient%03.0d" % id,
                         "patient%03.0d_frame%02.0d.nii.gz" % (id, patient_info[id]["ed"])))
        es_image = sitk.ReadImage(
            os.path.join(folder, "patient%03.0d" % id,
                         "patient%03.0d_frame%02.0d.nii.gz" % (id, patient_info[id]["es"])))
        old_spacing = np.array(ed_image.GetSpacing())[[2, 1, 0]]

        target_spacing[0] = old_spacing[0]

        patient_info[id]['ed_data'] = resize_image(sitk.GetArrayFromImage(ed_image).astype(float), old_spacing, target_spacing, 3).astype(
            np.float32)
        patient_info[id]['es_data'] = resize_image(sitk.GetArrayFromImage(es_image).astype(float), old_spacing, target_spacing, 3).astype(
            np.float32)

    return patient_info


def get_patients_spacing(folder='../training'):
    patients = range(1, 101)
    spacing = {}
    patient_info = {}
    for i, id in enumerate(patients):
        ...
        ed_image = sitk.ReadImage(
            os.path.join(folder, "patient%03.0d" % id,
                         "patient%03.0d_frame%02.0d.nii.gz" % (id, patient_info[id]["ed"])))
        spacing[id] = np.array(ed_image.GetSpacing())[[2, 1, 0]]
    return spacing
