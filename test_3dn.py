    ...
    for i in range(len(X)):
        ...
        for k in range(stacks_count):
            if k == 0 and X_heart_locator.shape[0] < SLICES:
                ...
                X_stack[(SLICES - X_heart_locator.shape[0]) // 2:X_heart_locator.shape[0] + (
                            SLICES - X_heart_locator.shape[0]) // 2] = X_heart_locator
            ...
            if k == 0 and X_heart_locator.shape[0] < SLICES:
                pred_mi_heart_locator[:, yc_heart_locator - HEIGHT_UNET // 2:yc_heart_locator + HEIGHT_UNET // 2,
                xc_heart_locator - WIDTH_UNET // 2:xc_heart_locator + WIDTH_UNET // 2, :] += ((
                                                                                                          y_heart_locator_predicted + yh_heart_locator_predicted + yv_heart_locator_predicted + yhv_heart_locator_predicted) / 4)[
                                                                                             (SLICES -
                                                                                              X_heart_locator.shape[
                                                                                                  0]) // 2:
                                                                                             X_heart_locator.shape[
                                                                                                 0] + (SLICES -
                                                                                                       X_heart_locator.shape[
                ...
                pred_mi_heart_locator[k * SLICES:(k + 1) * SLICES,
                yc_heart_locator - HEIGHT_UNET // 2:yc_heart_locator + HEIGHT_UNET // 2,
                xc_heart_locator - WIDTH_UNET // 2:xc_heart_locator + WIDTH_UNET // 2, :] += (
                                                                                                         y_heart_locator_predicted + yh_heart_locator_predicted + yv_heart_locator_predicted + yhv_heart_locator_predicted) / 4
            else:
                pred_mi_heart_locator[-SLICES:, yc_heart_locator - HEIGHT_UNET // 2:yc_heart_locator + HEIGHT_UNET // 2,
                xc_heart_locator - WIDTH_UNET // 2:xc_heart_locator + WIDTH_UNET // 2, :] += (
                                                                                                         y_heart_locator_predicted + yh_heart_locator_predicted + yv_heart_locator_predicted + yhv_heart_locator_predicted) / 4
